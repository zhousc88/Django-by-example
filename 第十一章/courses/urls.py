...
urlpatterns = [
    ...
    url(r'^module/(?P<module_id>\d+)/conten/(?P<model_name>\w+)/(?P<id>\d+)/$',views.ContentCreateUpdateView.as_view(),
        name='module_content_update'),
    url(r'^content/(?P<id>\d+)/delete/$',views.ContentDeleteView.as_view(),name='module_content_delete'),
    url(r'module/(?P<module_id>\d+)/$',views.ModuleContenListView.as_view(),name='module_content_list'),
